...
#从svn拉取分支
def svn_checkout():
	branch = '%(srcRoot)s%(branchName)s'%(settings)
	print(branch)
	if os.path.exists(branch):
		print('分支已经存在，更新分支')
		cmd = 'svn update %s'%(branch)
	else:
		print('分支不存在，拉取分支')
		cmd = 'svn co %(svn)s%(branchName)s %(srcRoot)s%(branchName)s'%(settings)
	execute_cmd(cmd)
...
def package():
	...
	os.chdir(branch)
	cmd = '/opt/software/apache-maven-3.2.3/bin/mvn clean package -P live  -DSkipTests -Denv=live -Djava.home=' + os.environ['JAVA_HOME'] + '/jre'
	execute_cmd(cmd)

#备份原来的jar包
def bakup_project():
	source = '%(workDir)s/webapps/%(tarDir)s'%settings
	dest = '%(backupDir)s%(projectName)s'%(settings) + '.' + get_current_time() + '.tar.gz'
	if os.path.exists(source):
		os.chdir(settings['workDir'] + '/webapps')
		cmd = 'tar zcvf %(tarDir)s.tar.gz %(tarDir)s'%settings
		execute_cmd(cmd)
		print('备份文件：%s.tar.gz到%s'%(source, dest))
		shutil.move(source + '.tar.gz',dest)
	else:
		print('文件不存在，不需要备份')
