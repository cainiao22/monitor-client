...
def package():
	...
	os.chdir(branch)
	cmd = '/opt/software/apache-maven-3.2.3/bin/mvn clean package -DSkipTests -Denv=live -Djava.home=' + os.environ['JAVA_HOME'] + '/jre'
	execute_cmd(cmd)
...
if __name__ == '__main__':
	settings = {
		'git': 'git@git.iqdnet.cn:bigdata/ds.git', #svn地址
		'srcRoot': '/opt/web/project/yushanfang/', #源代码存放位置
		'branchName': 'master', #分支名称
		'workDir': '/opt/web/workDir/yushanfang', #生产环境项目的根路径
		'backupDir':'/opt/web/workDir/yushanfang/backup/',  #备份地址
		'targetName':'ds.war',
		'projectName':'ds.war',   #webapps下的名称
		'start':'start.sh',
		'stop':'shutdown.sh',
	}
	checkout()
